...
impl<USCI: SpiUsci> SpiConfig<USCI, ClockSet> {
    ...
    >(
        ...
        cs: STE,
        delay: SysDelay,
    ) -> SpiPeriphHwCs<USCI, STE> {
        self.configure_hw();
        SpiPeriphHwCs(SpiPeriph(PhantomData), cs, delay)
    }
...
pub struct SpiPeriphHwCs<USCI: SpiUsci, STE: Into<USCI::STE>>(SpiPeriph<USCI>, STE, SysDelay);
...
mod ehal1 {
    ...
    impl<USCI: SpiUsci, STE: Into<USCI::STE>> ErrorType for SpiPeriphHwCs<USCI, STE> {
        type Error = SpiErr;
    }
    impl<USCI: SpiUsci, STE: Into<USCI::STE>> SpiDevice for SpiPeriphHwCs<USCI, STE> {
