...
use crate::{
    ...
    hw_traits::Steal, sac::{DacLoadTrigger, DacRef, PowerMode}
};
...
pub trait SacPeriph {
    ...
    /// Opamp output pin
    type OutputPin;
    fn configure_sacoa(psel: PSel, nsel: NSel, pm: PowerMode);
    fn configure_sacpga(gain: u8, mode: MSel);
    fn configure_dac(load_condition: DacLoadTrigger, vref: DacRef);
...
macro_rules! impl_sac_periph {
    ($SAC: ident, $port: ident, $inPp: ident, $inNp: ident, $outp: ident, // Register block, port, pos_in, neg_in, out
        ...
        impl SacPeriph for $SAC {
            ...
            fn configure_dac(lsel: DacLoadTrigger, rf: DacRef) {
                unsafe{
                    let sac = $SAC::steal();
                    sac.$sacXdac.write(|w| w
                        .dacsref().bit(rf.into())
                        .daclsel().bits(lsel as u8)
                        .dacdmae().clear_bit()
                        .dacie().clear_bit()
                        .dacen().set_bit()
                    );
