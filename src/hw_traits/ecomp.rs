...
use crate::{
    ecomp::{ComparatorDac, DacBufferSel, DacVRef, FilterStrength, Hysteresis, NegativeInput, OutputPolarity, PositiveInput, PowerMode}, 
...
pub trait ECompPeriph: Steal {
    ...
    type SACn;
    fn cpxdacctl(enable: bool, vref: DacVRef, buf_mode: DacBufferMode, buf: DacBufferSel);
    fn set_buf1_val(buf: u8);
    fn set_buf2_val(buf: u8);
    fn set_dac_buffer_mode(mode: DacBufferMode);
    fn select_buffer(sel: DacBufferSel);
...
pub enum DacBufferMode {
    ...
    Hardware,
    /// In software mode the DAC count can be switched between CPDACBUF1 and CPDACBUF1 by software
    Software,
}
...
macro_rules! impl_ecomp {
    ($COMP: ident, 
        ...
        impl ECompPeriph for $COMP{
            ...
            fn check_gpio_used() -> (bool, bool) {
                let comp = unsafe { $COMP::steal() };
                let p_in = comp.$cpctl0.read().cppsel().bits();
                let n_in = comp.$cpctl0.read().cpnsel().bits();

                (p_in == 0b000 || n_in == 0b000, p_in == 0b001 || n_in == 0b001)
            }
        }
    };
}
